...
function tweet(opts, callback) {
  ...
  lastIdFileName = process.cwd() + '/last-ids/' + botName + 'last-id.json',
  ...
  Twitter.get('search/tweets', searchObj, function(err, data, response) {
    if (!err) {
      if (data.statuses.length) {
        ...
        constructTweets(data, opts.mentioned_user, opts.intended_user, opts.bot_name, lastIdFileName, lastIdStr, callback);
...
function constructTweets(data, mentioned_user, intended_user, bot_name, lastIdFileName, lastIdStr, callback) {
  ...
  for (var i = 0; i < statuses.length; i++) {
    if (statuses[i].id_str > lastIdStr) {
      ...
      if (bot_name !== statuses[i].user.screen_name) {
